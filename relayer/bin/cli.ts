...
program
  ...
  .description('Generate monthly reports for merchants and upload to Filecoin via Storacha')
  ...
  .action(async (options) => {
    ...
    const { isStorachaEnabled } = await import('../src/lib/storacha.js')
    const { generateAndUploadReport, generateAndSaveReport } = await import('../src/reports/upload.js')
    const useIpfs = isStorachaEnabled()
    ...
    let merchants: Array<{ address: string }>
    if (options.merchant) {
      merchants = [{ address: options.merchant.toLowerCase() }]
    } else {
      merchants = await listMerchants(config.databaseUrl)
    }

    if (merchants.length === 0) {
      console.log('\n✅ No merchants found. Nothing to generate.\n')
      return
    }

    console.log(`\nGenerating ${period} reports for ${merchants.length} merchant(s) on ${chainConfig.name}${useIpfs ? '' : ' (local only)'}...\n`)
    ...
    for (const merchant of merchants) {
      try {
        const { cid } = useIpfs
          ? await generateAndUploadReport(config.databaseUrl, chainConfig.chainId, merchant.address, period)
          : await generateAndSaveReport(config.databaseUrl, chainConfig.chainId, merchant.address, period)

        console.log(`  ✅ ${merchant.address}${cid ? `: ${cid}` : ' (local)'}`)
        success++
      } catch (err) {
